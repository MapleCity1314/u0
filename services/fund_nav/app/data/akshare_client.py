...
from ..core.config import CACHE_TTL_SEC
from .cache import TTLCache

REQUEST_TIMEOUT = 25

_fund_est_cache = TTLCache(CACHE_TTL_SEC)
_stock_spot_cache = TTLCache(CACHE_TTL_SEC)
_etf_spot_cache = TTLCache(CACHE_TTL_SEC)
_industry_spot_cache = TTLCache(CACHE_TTL_SEC)
_index_spot_cache = TTLCache(CACHE_TTL_SEC)
...
def get_fund_value_estimation() -> Optional[pd.DataFrame]:
    ...
    def fetch():
        return _clean_df(call_with_timeout(ak.fund_value_estimation_em, timeout=REQUEST_TIMEOUT))

    return _fund_est_cache.get_or_set(fetch)
...
def get_stock_spot() -> Optional[pd.DataFrame]:
    def fetch_em():
        return _clean_df(call_with_timeout(ak.stock_zh_a_spot_em, timeout=REQUEST_TIMEOUT))

    def fetch_alt():
        return _clean_df(call_with_timeout(ak.stock_zh_a_spot, timeout=REQUEST_TIMEOUT))

    cached = _stock_spot_cache.get()
    ...
    if spot is not None:
        return _stock_spot_cache.set(spot)
    return cached
...
def get_etf_spot_return_map() -> Optional[pd.Series]:
    ...
    cached = _etf_spot_cache.get()
    if cached is not None:
        return cached

    try:
        spot = _clean_df(call_with_timeout(ak.fund_etf_spot_em, timeout=REQUEST_TIMEOUT))
    except Exception:
        return cached
    ...
    return _etf_spot_cache.set(pd.Series(dict(out)))
...
def get_index_spot_pct_change() -> Optional[pd.Series]:
    ...
    if data is None:
        return cached
    return _index_spot_cache.set(data)


def get_industry_spot_pct_change() -> Optional[pd.Series]:
    if not hasattr(ak, "stock_board_industry_spot_em"):
        return None

    def fetch():
        spot = _clean_df(call_with_timeout(ak.stock_board_industry_spot_em, timeout=REQUEST_TIMEOUT))
        name_col = None
        pct_col = None
        for c in spot.columns:
    ...
    cached = _industry_spot_cache.get()
    if cached is not None:
        return cached
    try:
        data = fetch()
    ...
    if data is None:
        return cached
    return _industry_spot_cache.set(data)
...
def get_stock_industry(code: str) -> Optional[str]:
    if not hasattr(ak, "stock_individual_info_em"):
        return None
    try:
        info = _clean_df(call_with_timeout(ak.stock_individual_info_em, kwargs={"symbol": code}))
    except Exception:
        return None
...
def estimate_fund(code: str, index_code: str | None = None) -> dict:
    ...
    if est_override is not None and est_override["kind"] == "value" and last_nav:
